...
///source for shader program - based on sf::Shader///
...
namespace ml
{
	...
	////////////////////////////////////////////////////////////
	void ShaderProgram::setParameter(const std::string& name, const glm::vec3& v)
	{
		setParameter(name, v.x, v.y, v.z);
	}
	...
	{
		setParameter(name, glm::value_ptr(matrix));
	}
		...
		if (m_shaderProgram)
		{
			...
			if (location != -1)
			{
				...
				if (it == m_textures.end())
				{
					...
					m_textures[location] = std::make_pair(&texture, 0u);
				}
				else
				{
	...
	void ShaderProgram::setParameter(const std::string& name, sf::Uint32 glTextureId)
	{
		if (m_shaderProgram)
		{
			ensureGlContext();
			...
			if (location != -1)
			{
				...
				if (it == m_textures.end())
				{
					...
					m_textures[location] = std::make_pair(nullptr, glTextureId);
				}
				else
				{
					// Location already used, just replace the texture
					it->second.second = glTextureId;
				}
			}
		}
	}
	...
	////////////////////////////////////////////////////////////
	void ShaderProgram::addVertexAttribute(const std::string& name)
	{
	...
	void ShaderProgram::bindTextures() const
	{
		...
		for (std::size_t i = 0; i < m_textures.size(); ++i)
		{
			...
			//check if we're binding a sf::Texture or openGL texture
			if (it->second.first)
				sf::Texture::bind(it->second.first);
			else
				glBindTexture(GL_TEXTURE_2D, it->second.second);
			++it;
		}
	...
	int ShaderProgram::getAttribLocation(const std::string& name)
	{
		...
		else
		{
			...
			{
				sf::err() << "Attribute \"" << name << " not found in shader" << std::endl;
			}
			return location;
		}
	}
